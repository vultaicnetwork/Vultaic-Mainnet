...
const std::string VlcGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString VlcGUI::DEFAULT_WALLET = "~Default";

VlcGUI::VlcGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void VlcGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a vlc address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and vlc: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to vlc addresses"));
...
//    restrictedAssetAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your vlc addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified vlc addresses"));
    ...
    openAction->setStatusTip(tr("Open a Vlc: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible vlc command-line options").arg(tr(PACKAGE_NAME)));
...
void VlcGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/vlccointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* vlcLabelLayout = new QVBoxLayout(toolbarWidget);
        vlcLabelLayout->addWidget(label);
        vlcLabelLayout->addWidget(toolbar);
        vlcLabelLayout->setDirection(QBoxLayout::TopToBottom);
        vlcLabelLayout->addStretch(1);
        ...
        // Get the latest Vlc release and let the user know if they are using the latest version
        // Network request code for the header widget
...
void VlcGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool VlcGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool VlcGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void VlcGUI::setWalletActionsEnabled(bool enabled)
{
...
void VlcGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void VlcGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void VlcGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void VlcGUI::showDebugWindowActivateInformation()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_INFO);
    showDebugWindow();
}

void VlcGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void VlcGUI::showDebugWindowPeerList()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_PEERS);
    showDebugWindow();
}

void VlcGUI::showDebugWindowNetworkTraffic()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_GRAPH);
    showDebugWindow();
}

void VlcGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void VlcGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void VlcGUI::openClicked()
{
...
void VlcGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void VlcGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void VlcGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void VlcGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void VlcGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void VlcGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** VLC START */
void VlcGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void VlcGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void VlcGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void VlcGUI::gotoRestrictedAssetsPage()
{
...
void VlcGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to vlc network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void VlcGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void VlcGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void VlcGUI::updateHeadersSyncProgressLabel()
{
...
void VlcGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void VlcGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Vlc"); // default title
...
void VlcGUI::changeEvent(QEvent *e)
{
...
void VlcGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void VlcGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void VlcGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "vlc")
        msg += tr("Amount: %1\n").arg(VlcUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(VlcUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void VlcGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void VlcGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool VlcGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool VlcGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void VlcGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void VlcGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void VlcGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void VlcGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void VlcGUI::detectShutdown()
{
...
void VlcGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void VlcGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void VlcGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(VlcGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
static bool ThreadSafeMnemonic(VlcGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void VlcGUI::subscribeToCoreSignals()
{
...
void VlcGUI::unsubscribeFromCoreSignals()
{
...
void VlcGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void VlcGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<VlcUnits::Unit> units = VlcUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const VlcUnits::Unit unit : units)
    {
    	#ifndef QTversionPreFiveEleven
        	max_width = qMax(max_width, fm.horizontalAdvance(VlcUnits::name(unit)));
        #else
        	max_width = qMax(max_width, fm.width(VlcUnits::name(unit)));
        #endif
    }
...
{
    menu = new QMenu(this);
    for (VlcUnits::Unit u : VlcUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(VlcUnits::name(u)), this);
...
{
    setText(VlcUnits::name(newUnits));
}
...
void VlcGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=vlcBTC"));
    networkManager->get(*request);
}

#ifdef ENABLE_WALLET
void VlcGUI::mnemonic()
{
...
void VlcGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/JustAResearcher/Vlc/releases"));
