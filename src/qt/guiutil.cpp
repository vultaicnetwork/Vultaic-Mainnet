...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a vlc address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new VlcAddressEntryValidator(parent));
    widget->setCheckValidator(new VlcAddressCheckValidator(parent));
}
...
bool parseVlcURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no vlc: URI
    if(!uri.isValid() || uri.scheme() != QString("vlc"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!VlcUnits::parse(VlcUnits::vlc, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseVlcURI(QString uri, SendCoinsRecipient *out)
{
    // Convert vlc:// to vlc:
    //
    //    Cannot handle this later, because vlc:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("vlc://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "vlc:");
    }
    QUrl uriInstance(uri);
    return parseVlcURI(uriInstance, out);
}

QString formatVlcURI(const SendCoinsRecipient &info)
{
    QString ret = QString("vlc:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(VlcUnits::format(VlcUnits::vlc, info.amount, false, VlcUnits::separatorNever));
        paramCount++;
    }
...
bool openVlcConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", VLC_CONF_FILENAME));

    /* Open vlc.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Vlc.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Vlc (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Vlc (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "vlc.desktop";
    return GetAutostartDir() / strprintf("vlc-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a vlc.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Vlc\n";
        else
            optionFile << strprintf("Name=Vlc (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the vlc app
...
bool GetStartOnSystemStartup()
{
    CFURLRef vlcAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (vlcAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, vlcAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add vlc app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, vlcAppUrl, nullptr, nullptr);
    }
