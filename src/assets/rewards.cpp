...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for vlc
    ...
    if (p_rewardSnapshot.strDistributionAsset != "vlc") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is vlc with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using vlc differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "vlc") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Vlc address (already validated during ownership snapshot creation)
