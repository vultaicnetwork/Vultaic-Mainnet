...
find ${DISTNAME}/bin -type f -executable -exec ../Vlc/contrib/devtools/split-debug.sh {} {} {}.dbg \;
find ${DISTNAME}/lib -type f -exec ../Vlc/contrib/devtools/split-debug.sh {} {} {}.dbg \;
...
cd ~/
rm -rf win64
cp -rf Vlc/contrib/windeploy ~/sign/win64
cd ~/sign/win64/windeploy
mkdir -p unsigned
mv ~/Vlc/vlc-*setup-unsigned.exe unsigned/
...
cd ~/
rm -rf win32
cp -rf Vlc/contrib/windeploy ~/sign/win32
...
mkdir -p ~/Vlc/depends/SDKs
cp ~/MacOSX10.11.sdk.tar.gz ~/Vlc/depends/SDKs/MacOSX10.11.sdk.tar.gz
cd ~/Vlc/depends/SDKs && tar -xf MacOSX10.11.sdk.tar.gz 
