...
The VLC Core daemon can be configured to act as a trusted "border
router", implementing the vlc wire protocol and relay, making
...
The ZeroMQ feature in VLC Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building vlcd:
...
For instance:
    ...
    $ vlcd -zmqpubhashtx=tcp://127.0.0.1:28766 \
               -zmqpubrawtx=ipc:///tmp/vlcd.tx.raw
...
These options can also be provided in vlc.conf.
...
From the perspective of vlcd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into vlcd directly. Furthermore, no information is
...
using. VLCd appends an up-counting sequence number to each
