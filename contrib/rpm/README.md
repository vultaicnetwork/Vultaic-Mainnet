...
The RPM spec file provided here is for Vlc-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://vultaicnetwork.com//bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Vlc git repository](https://github.com/vultaicnetwork/Vultaic-Mainnet) but are not part of
...
Currently only `Source10` is used, for the example `vlc.conf` file.
...
in the vlc git repository. At present only an SVG version of the Vlc
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the vlc
...
needed when 0.12.1 is released, a proper fix is already in the Vlc git
master branch.
...
idea to build Vlc Core against a static version of that library compiled
...
The `vlc.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `vlc-server` package. At this point, most RPM based
...
The `vlc-server` package should have SELinux support. How to properly do
...
Vlc-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/vlc/). He has been
packaging Vlc for Fedora at least since 2012.
...
4. I renamed the `vlc` package that contains the Qt GUI to `vlc-core` as
that appears to be how the general population refers to it, in contrast to
`vlc-xt` or `vlc-classic`. I wanted to make sure the general population
